...
def dump_example(scene, args):
    ...
    for scene_data in scene_list:
        ...
        for sample in data_loader.get_scene_imgs(scene_data):
            ...
            imageio.imwrite(dump_img_file, img, format='jpg')
        ...
        if len(dump_dir.files('*.jpg')) < 3:
            # 删掉少于3张的视频序列
            dump_dir.parent.rmtree()


def main():
    args.dump_root = Path(args.dump_root)
    args.dump_root.mkdir_p()  # 新建文件夹，若已存在不引起Error

    global data_loader

    if args.dataset_format == 'kitti':
        from data.kitti_raw_loader import KittiRawLoader
    ...
    if args.dataset_format == 'cityscapes':
        from data.cityscapes_loader import cityscapes_loader
